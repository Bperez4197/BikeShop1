    ...
    public class InvoiceAdapter
    {
        ...
        public List<Invoice> GetAll()
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(_connectionString))
            {
                ...
                command.CommandText = "SELECT InvoiceId, CustomerId, Amount, InvoiceDate FROM Invoice";
        ...
        public List<Invoice> GetByCustomerId(int customerId)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(_connectionString))
            {
                // Create the command
                SQLiteCommand command = connection.CreateCommand();
                command.CommandText = "SELECT InvoiceId, CustomerId, Amount, InvoiceDate FROM Invoice WHERE CustomerId = " + customerId.ToString();
              connection.Open();
                SQLiteDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
        ...
        public bool InsertInvoice(Invoice invoice)
        {
            ...
            {
                SQLiteCommand command = connection.CreateCommand();
                command.CommandText = "INSERT INTO Invoice (CustomerId, Amount, InvoiceDate) VALUES ("
                + invoice.CustomerId + "," + invoice.Amount + ",'" + invoice.InvoiceDate + "')";
                connection.Open();
        ...
        public bool UpdateInvoice(Invoice invoice)
        {
            ...
            {
                SQLiteCommand command = connection.CreateCommand();
                command.CommandText = "UPDATE Invoice SET Amount = " + invoice.Amount + ", InvoiceDate = '"
